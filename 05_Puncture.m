...
% NOTE: (1)need 1000 bits AFTER puncturing, so 1333 bits before puncturing
%       (2) 1333 bits BEFORE 1:4 puncturing, AFTER encoding
%       (3) 666 bits before encoding to acehieve 1332 encoded bits
%       (4) add 1 dummy bit after encoding to achieve 1333 bits
%       (5) dummy bit automatically removed after puncturing
n = 666; %OUTPUT OF CONVENC = 1332 (derived in class)
...
OUTPUTconvenc = [OUTPUTconvenc 0]; %ADD DUMMY BITS

%PUNCTURING(for every 4 bits, 3rd bit punctured):
PUNCTURE = (OUTPUTconvenc);
PUNCTURE(3:4:end) = []; %FOR EVERY 6 BITS, DELETE 2 BITS(from lecture)
PUNCTURE = PUNCTURE(1:end);

%INTERLEAVE(10x5 interleaver specified):
for i_=1:50 % row 50 x col 20 ; cut vector 20 each; total 50 block
    INTERLEAVE(i_,:) = PUNCTURE(20*(i_-1)+1:20*i_);
end
INFORMATION_butcooler = INTERLEAVE(:)';

%BURST CONSTRUCTION (append all):
burst = [guard_bits, unique_bits, INFORMATION_butcooler, guard_bits];
...
%DE-INTERLEAVING:
for j_= 1 : 20 % take 50 rows each; put 20 column vectors
    DEINTERLEAVE(:,j_) = INFO_bits(50*(j_-1)+1:50*j_);
end
DEINTERLEAVEoutput = DEINTERLEAVE'; 
DEINTERLEAVEoutput = DEINTERLEAVEoutput(:)';

%DE-PUNCTURING:
dummybit = 0; %idk it works
DEPUNCTURE = ones(1,1334);
DEPUNCTURE(1:4:end) = [DEINTERLEAVEoutput(1:3:end)]; 
DEPUNCTURE(2:4:end) = [DEINTERLEAVEoutput(2:3:end) dummybit]; 
DEPUNCTURE(4:4:end) = [DEINTERLEAVEoutput(3:3:end)]; 
Demod_pun = [DEPUNCTURE 0];
